...
def CreateODMatrix(
    ...
    def EuclidCall(source_list, dest_list, source_points, dest_points):
        ...
        df = pd.DataFrame(data=distmatrix, columns=dest_list, index=source_list)
        df = df.stack(level=-1)
        df.columns = ["O", "D", "DIST"]
        return df
    ...
    def Call(O_list, D_list, i, O_IDs, D_IDs, header):
        ...
        if not call_type == "OSRM":
            ...
            # Build request string
            request = (
                header + data + "?sources=" + sources + "&destinations=" + destinations
            )
    ...
    print(
        f"length of sources list {len(sources_list)}, and destinations list is {len(dests_list)}"
    )
...
def MarketAccess(
    ...
    for lamdar in lambder_list:
        output["%s_%s" % ("d", lamdar)] = (
            new.loc[new["DIST"] > 0]
            .groupby("O_UID")
            .apply(lambda x: market_access(x, lamdar))
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="ALL",
        action="store_true",
        help="Set if you want to run both OD Matrix Calculation and Market Access",
    )
    parser.add_argument(
        "-od", dest="OD", action="store_true", help="Run OD Matrix calculation only"
    )
    parser.add_argument(
        "-ma", dest="MA", action="store_true", help="Run Market Access Calculation"
    )

    parser.add_argument(
        "-s", dest="SOURCES_FILE", action="store", help="Sources Points as CSV"
    )
    parser.add_argument(
        "-d", dest="DESTINATION_FILE", action="store", help="Destination Points as CSV"
    )
    parser.add_argument(
        "-matrix",
        dest="MATRIX_FILE",
        action="store",
        help="if running only market access, define Matrix csv here",
    )

    parser.add_argument(
        "-outputOD", dest="OD_FILE", action="store", help="Output csv for OD Matrix"
    )
    parser.add_argument(
        "-outputMA",
        dest="MA_FILE",
        action="store",
        help="Output csv for market access results",
    )
    ...
    parser.add_argument(
        ...
        action="store",
        default="Lat",
        help="Name of Latitude coordinates in both sources and dests",
    )
    parser.add_argument(
        "--lon",
        dest="LON_NAME",
        action="store",
        default="Lon",
        help="Name of Longitude coordinates in both sources and dests",
    )
    parser.add_argument(
        "--id",
        dest="UID",
        action="store",
        default="ID",
        help="unique identifier used in both sources and dests",
    )
    parser.add_argument(
        "--Pop",
        dest="POPFIELD",
        action="store",
        default="Pop",
        help="Field in input files defining population for sources and destinations",
    )
    parser.add_argument(
        "--osrm",
        dest="OSRMHEADER",
        action="store",
        default="",
        help="optional parameter to set OSRM source",
    )
    parser.add_argument(
        "--sleep",
        dest="SLEEPTIME",
        action="store",
        default=3,
        help="When making calls to OSRM, a sleep time is required to avoid DDoS",
    )
